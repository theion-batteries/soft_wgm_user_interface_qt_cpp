...
heatingView::heatingView(Ui::MainWindow* uiPtr)
{
    ...
    connect(&heatControll, &heatingController::heaterConnected, this, [this]() {
        ...
        auto func = QtConcurrent::run(&heatingController::updateLcdTemperature, &heatControll, ui->lcd_temperature);
        }, Qt::QueuedConnection);
    ...
    connect(ui->heat_input_cmd, &QLineEdit::returnPressed, this, [this]() {
        ...
        auto func = QtConcurrent::run(&heatingController::updateLabelHeaterResponse, &heatControll, ui->heat_cmd_response, inputCmd);
        }, Qt::QueuedConnection);
