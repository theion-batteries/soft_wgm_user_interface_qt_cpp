...
// connect execute single proc
    connect(ui->execute_process, &QAbstractButton::pressed, [this]() {
        auto func = QtConcurrent::run([this]() {mainControll.on_execute_process_clicked();});
    mainControll.updateLcdTime(ui->time_processes);
    mainControll.updateProgressBar(ui->progressBar);
        });
// connect execute all proc
    connect(ui->execute_all_processes, &QAbstractButton::pressed, [this]() {
